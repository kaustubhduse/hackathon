...
function HomePage() {
  ...
  // Refs for elements to animate on scroll
  ...
  // Intersection Observer setup to trigger animations on scroll
  useEffect(() => {
    const options = {
      root: null,
      rootMargin: '0px',
    ...
    // Observe the sections to add animation
    if (mainContentRef.current) observer.observe(mainContentRef.current);
    if (statsRef.current) observer.observe(statsRef.current);
    if (whyParticipateRef.current) observer.observe(whyParticipateRef.current);
  ...
  return (
    <div>
      ...
      <div className="bg-[rgba(0,49,69,1)]">
        ...
        <div
          ref={mainContentRef}
          className="flex flex-col lg:flex-row space-y-5 lg:space-y-0 lg:space-x-[5%] justify-center items-center p-5 opacity-0 transition-opacity duration-700"
        >
          <div className="text-left w-full lg:w-[40%] py-8 lg:py-[98px] slide-up">
            ...
            <Link
              to="/create-challenge"
              className="text-[rgba(0,49,69,1)] mt-4 lg:mt-[7%] bg-white py-2 px-3 rounded-xl font-bold inline-block create-challenge-button"
            >
              Create Challenge
            </Link>
          </div>
      ...
      <div
        ref={statsRef}
        className="flex flex-col lg:flex-row bg-[rgba(0,42,59,1)] justify-center p-6 lg:p-12 lg:space-x-[12%] space-y-6 lg:space-y-0 border-t-2 border-t-gray-400 opacity-0 transition-opacity duration-700"
      >
      ...
      <div
        ref={whyParticipateRef}
        className="px-6 lg:px-12 py-8 opacity-0 transition-opacity duration-700"
      >
...
function StatCard({ image, value, description }) {
  return (
    <div className="flex items-center space-x-4 lg:space-x-6 stat-card">
